...
ui = navbarPage("DataViz",theme = shinytheme("cerulean"),
           ...
           p("See the FAQ section for more details."),
                             ...
                             checkboxGroupInput("ratioSumsToCreate",h5("Ratio of Intensity Sum Normalized per Nucleus")),
                             actionLink("selectRatioSums","Select/Clear All Sum Ratios"),
                             checkboxGroupInput("ratioSumsPerGroupToCreate",h5("Ratio of Intensity Sum Normalized by Group")),
                         ...
                         mainPanel(
                             tabsetPanel(type = "tabs",
                                         ...
                                         tabPanel("Outliers",
                                                  ...
                                                  selectInput(inputId = "filterChOI","per Channel",choices = NULL),
                                                  actionButton("additionalFilter", "Apply Filter"),
                                                  actionButton("cancelFilter", "Cancel Last Filter*"),
                                                  actionButton("cancelAllFilter", "Clear All Filters"),
                                                  p(),
                                                  p("*Filters are stackable. Remove the last applied filter. "),
                                                                      h2("Binning"),
                                                                      tabPanel("Subset",
                                                  #p("Bin the data defined by thresholds"),
                                                  selectInput(inputId = "binningVariable","Variable for Bin Creation",choices = list("Select Data First"="NormSum"),selected = "NormSum"),
                                                  #radioButtons(inputId = "rangeOrGroups", p("Create bins by thresholds"), c("Thresholds (e.g., '0,1,5,10')"="threshold"), selected = "threshold"),
                                                  textInput("binCuts","Create bins by thresholds (e.g., '0,1,5,10')",value="0,1"),
                                                  actionButton("addBins", "Create Bins"),
                                                  actionButton("removeBins","Clear Bins")
                                         )
                                                                      ),
                                                                  mainPanel(
